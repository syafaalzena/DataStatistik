...
<head>
    ...
    <title>Rekap Tahunan Budidaya Provinsi Aceh</title>
    ...
    <style>
        ...
        .filter-card .form-select { border-radius: 8px; border: 1px solid #e2e8f0; }
...
<div class="container pb-5">
    ...
    <div class="d-flex align-items-center gap-3 mb-4">
        ...
        <div>
            <h2 class="fw-bold mb-1">Rekap Tahunan Budidaya Provinsi Aceh</h2>
            <p class="text-muted mb-0">Pilih tahun untuk melihat rekap sarana semua kabupaten.</p>
        </div>
    </div>

    {{-- FILTER --}}
    <form method="GET" action="{{ route('budidaya.rekapTahunan') }}" class="filter-card mb-4">
        ...
        <div class="mt-3">
            <a href="{{ route('budidaya.rekapTahunan.export', request()->query()) }}" class="btn-excel">
    ...
    <div class="rekap-table">
        @if($rekap->isEmpty())
            <div class="empty-state">Belum ada data sarana untuk tahun {{ $tahun }}.</div>
        @else
            <table class="table table-hover mb-0">
                <thead>
                    <tr>
                        <th style="width: 35%">Kabupaten / Jenis Budidaya</th>
                ...
                <tbody>
                    @foreach($rekap as $kab)
                        <tr class="row-kabupaten">
                            ...
                            <td class="text-end">{{ number_format($kab['total_rtp']) }}</td>
                            <td class="text-end">{{ number_format($kab['total_pembudidaya']) }}</td>
                            <td class="text-end">{{ number_format($kab['total_luas_lahan']) }}</td>
                        </tr>
                        @foreach($kab['jenis_list'] as $item)
                            <tr>
                                <td class="ps-4">{{ $item['jenis']->nama_jenis ?? '-' }}</td>
                                <td class="text-end">{{ number_format($item['jumlah_rtp']) }}</td>
                                <td class="text-end">{{ number_format($item['jumlah_pembudidaya']) }}</td>
                                <td class="text-end">{{ number_format($item['luas_lahan']) }}</td>
                            </tr>
                        @endforeach
                    @endforeach
                </tbody>
