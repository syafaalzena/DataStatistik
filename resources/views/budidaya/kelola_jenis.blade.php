...
<head>
    ...
    <title>Kelola Jenis Budidaya</title>
    ...
    <style>
        ...
        .jenis-item:hover { background: #f8fafc; }

        .btn-delete-icon { background: none; border: none; color: #dc2626; font-size: 13px; font-weight: 600; }
...
<div class="container pb-5" style="max-width: 640px;">
    ...
    <div class="d-flex align-items-center gap-3 mb-4">
        <a href="{{ route('budidaya.input', $kabupatenId) }}" class="back-btn">
            <img src="{{ asset('images/back.png') }}" alt="Back" width="22" height="22">
        </a>
        <div>
            <h2 class="fw-bold mb-1">Kelola Jenis Budidaya</h2>
            <p class="text-muted mb-0">Contoh: Tambak, Kolam, Keramba, Jaring Apung, dsb.</p>
        </div>
    </div>
    ...
    <div class="panel-card mb-4">
        <h5 class="fw-bold mb-3">Tambah Jenis Budidaya</h5>
        <form method="POST" action="{{ route('budidaya.jenis.store', $kabupatenId) }}" class="d-flex gap-2">
            @csrf
            <input type="text" name="nama_jenis" class="form-control" placeholder="Contoh: Tambak" required>
            <button type="submit" class="btn-dark-custom text-nowrap">Simpan</button>
        </form>
    </div>

    <div class="panel-card">
        <h5 class="fw-bold mb-3">Daftar Jenis Budidaya ({{ $jenis->count() }})</h5>

        @if($jenis->isEmpty())
            <div class="empty-state">Belum ada jenis budidaya untuk kabupaten ini.</div>
        @else
